  ...
  if (node.uncategorisedChildren.length === 0) {
    ...
    node.uncategorisedChildren = node.uncategorisedChildren.filter((value) => {
      if (value === "") {
        ...
        node.children.push(
          new Node(value, node, terminator = true)
        );
      } else {
        return value;
      }
...
fs.readFile("regex.txt", "utf-8", (err, data) => {
  ...
  let finalRegex = "^" + primitivesRegex + "$";
  // let finalRegex = "(\\\\\\\\)" + primitivesRegex + "(?=[^a-zA-Z@])";
  console.log(finalRegex);

  fs.writeFile('regex-results.txt', finalRegex, (err) => {
    if (err) throw err;
